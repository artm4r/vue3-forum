...
export default {
    ...
    actions: {
        async createPost ({ commit, state, rootState }, post) {
            ...
            commit('setItem', { resource: 'posts', item: newPost }, { root: true }) // set the post
            commit('threads/appendPostToThread', { childId: newPost.id, parentId: post.threadId }, { root: true }) // append post to thread
            if (!post.firstInThread) {
                commit('threads/appendContributorToThread', { childId: rootState.auth.authId, parentId: post.threadId }, { root: true }) // append contributor to thread
            }
        },
        ...
        fetchPost: makeFetchItemMutation({ resource: 'posts'}),
        fetchPosts: makeFetchItemsMutation({ resource: 'posts'}),
    },
    mutations: {
